...
<body style="background: #eeeeee;">
          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-body">
                ...
                <div class="row mb-3">
                  <b>Consultor, por favor, estime um percentual de aprontamento para a atividade (% Feita).</b>
                  <br />
                </div>
                ...
                <div class="row mb-3">
                  <div class="col-2">
                    ...
                    <div class="invalid-feedback">
                      <span id=""> Data fora do período permitido. </span>
                    </div>
                  </div>
                  <div class="col-3">
                ...
                <table style="text-align: center;" class="table table-sm table-bordered" id="tableAtividadesColaboradorDia">
                  <thead>
                    <tr style="text-align: center;">
                      <th style="width: 30%;"> Plano de Serviço</th>
                ...
                <table style="text-align: center;" class="table table-sm table-bordered" id="tableApontamentoHorasDaAtividade">
                  <thead>
                    <tr style="text-align: center;">
                      ...
                      <th style="width: 50%;"> Descrição do apontamento </th>
                      <th style="width: 10%;"> Trabalho (h) </th>
                      <th style="width: 10%;"> Aprontado (%) </th>
                    </tr>
                  </thead>
                  <tbody>
  ...
  <script type="text/javascript">
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          for (var i = data.length - 1; i >= 0; i--) {
            ...
            html.push('<tr>');
            html.push('<td>' + data[i].PES_Nome + '</td>');
    ...
    $('#inputTextLCT_DATA').change(function() {
      checkIfDateCanToPoint($(this).val().split("/").reverse().join("-"));
    ...
    function checkIfDateCanToPoint(LCT_DATA) {
      $('#inputTextLCT_DATA').removeClass('is-invalid');
      dateOutOfRange = true;
      $.ajax({
        url: "<?php echo base_url(); ?>consultoria/apontarHoras/checkIfDateCanToPoint",
        type: 'POST',
        dataType: 'json',
        success: function(data) {
          if (LCT_DATA >= data.SVC_ApontaLimiteMaior || LCT_DATA <= data.SVC_ApontaLimiteMenor) {
    ...
    $('#inputTextLCT_HORAINICIO').change(function() {
      ...
      var isValidHoraFim = /^([0-1][0-9]|2[0-3]):([0-5][0-9])$/.test($('#inputTextLCT_HORAFIM').val());
      ...
      if (!isValid) {
        ...
        var startHour = $('#inputTextLCT_HORAINICIO').val();
        var endHour = $('#inputTextLCT_HORAFIM').val();

        if (startHour && endHour) {
          var LCT_TEMPO = (new Date(("1970-1-1 " + $('#inputTextLCT_HORAFIM').val()).replace(/-/g, '/')) - new Date(("1970-1-1 " + $('#inputTextLCT_HORAINICIO').val()).replace(/-/g, '/'))) / 1000 / 60 / 60;
          if (!isNaN(LCT_TEMPO)) {
    ...
    $('#inputTextLCT_HORAFIM').change(function() {
      ...
      var isValid = /^([0-1][0-9]|2[0-3]):([0-5][0-9])$/.test($(this).val());
      var isValidHoraInicio = /^([0-1][0-9]|2[0-3]):([0-5][0-9])$/.test($('#inputTextLCT_HORAINICIO').val());
      if (!isValid) {
        $(this).addClass('is-invalid');
        hourError = true;
      } else {
        hourError = false;
        $(this).removeClass('is-invalid');
        if ($('#inputTextLCT_HORAINICIO').val().length == 5 && $('#inputTextLCT_HORAINICIO').val() >= $(this).val() && !isValidHoraInicio) {
          hourError = true;
          $(this).addClass('is-invalid');
          $('#inputTextLCT_TEMPO').val("");
