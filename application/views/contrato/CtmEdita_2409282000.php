    ...
    <div id="main-wrapper">
        ...
        <div class="page-wrapper d-flex flex-column min-vh-100">
                        ...
                        <div class="card">
                            <div class="card-body">
                               <div class="row mb-3">
								...
								<!-- Nav tabs (duas abas: Produto/Serviço e Pagamento) -->
								<ul class="nav nav-tabs" role="tablist">
									...
									<li class="nav-item">
										<a class="nav-link" data-toggle="tab" href="#parcelasPagamento" role="tab">
											<span class="hidden-xs-down">Parcelas de Pagamento</span>
										</a>
									</li>
								</ul>

								<!-- Tab panes (conteúdo das abas) -->
								<div class="tab-content">
									<!-- Aba "Informação Produto/Serviço" -->
									<div class="tab-pane active p-3" id="informacaoProdServ" role="tabpanel">
										<div class="row mb-3">
											<p>Nenhum conteúdo definido para "Informação Produto/Serviço" nesta versão.</p>
										</div>
									</div>
									...
									<!-- Aba "Parcelas de Pagamento" -->
									<div class="tab-pane p-3" id="parcelasPagamento" role="tabpanel">
										<div class="row mb-12">
											<table id="ParcPagTable" class="table table-hover" style="width:100%">
												<thead>
													<tr>
														...
														<th id='coParcVcto'>Vencimento<br>Previsto</th>
														<th id='coParcCorI'>Correção<br>Índice</th>
														<th id='coParcCor%'>Correção<br>(%)</th>
														<th id='coParcValo'>Valor (R$)</br><span id="ParcPagSpanTotal" style="font-weight: bold; color: darkblue;">-</span></th>
													</tr>
												</thead>
            ...
            <footer class="footer text-center mt-auto">
                © 2019 wDiscover Ltda - CtmEdita: vs00.10 2409271655
            </footer>
        </div>
    </div>
...
;		document.getElementById('button-addon3').addEventListener('click', function() {
		...
		function ParcPagListar () {
				...
				"columns": [
					{ "data": "CTP_Codigo" },
					{
						...
						render: function(data, type, row) {
							// return '<textarea type="text" style="text-align: left;" rows="2" value="' + data + '" id="rowCTP_Descricao' + row.CTP_Codigo + '" /></textarea>';
							return '<textarea style="text-align: left;" rows="2" id="rowCTP_Descricao' + row.CTP_Codigo + '">' + data + '</textarea>';
						}
					},
					{
					...
					{
						"data": "CTP_CorrecaoIndice",
						render: function(data, type, row) {
							return '<input type="text" style="text-align: left;" rows="2" value="' + data + '" id="rowCTP_CorrecaoIndice' + row.CTP_Codigo + '" />';
						}
					},
					{
						"data": "CTP_CorrecaoPercent",
						className: "text-right",
						render: function(data, type, row) {
							return '<input type="text" style="text-align: right;" value="' + data + '" id="rowCTP_CorrecaoPercent' + row.CTP_Codigo + '" />';
						}
					},
					{
						"data": "CTP_Valor",
						className: "text-right",
						render: function(data, type, row) {
							return '<input type="text" style="text-align: right;" value="' + data + '" id="rowCTM_ValorTotal' + row.CTP_Codigo + '" />';
						}
					}
				],
				"columnDefs": [
				...
				/*
				 $("input[id^='rowCTM_ValorTotal']").maskMoney({
						prefix: 'R$ ',
						thousands: '.',
						decimal: '.',
						affixesStay: true,
				...
				*/
				...
				$("#ParcPagTable").on('change', "input[id^='rowCTM_ValorTotal']", function() {
					ParcPagSomar();
				});
		...
		function ParcPagSomar() {
			...
			$('#ParcPagTable').find('tr').slice(1).each(function() {				
				vParcPagSomarTotalVal += parseFloat($(this).find('td:eq(5) input').val()) || 0;
			});
			
			$('#ParcPagSpanTotal').text(
