...
<body style="background: #eeeeee;">
    <div id="main-wrapper">
        ...
        <div class="page-wrapper d-flex flex-column min-vh-100">
											...
											<table id="ParcPagTable" class="table table-hover" style="width:100%">
												<thead>
													<tr>
														...
														<th> <button id="btnNovaParcela" style="border: none; background: none; font-size: 14px; cursor: pointer;">
            ...
            <footer class="footer text-center mt-auto">
                © 2019 wDiscover Ltda - CtmEdita: vs00.13 2410041930
            </footer>
        </div>
    </div>
    ...
    <script type="text/javascript">
		...
		function validarCampos() {
			...
			if ($('#inputTextCTM_Descricao').val() === '') {
				Swal.fire({
					title: 'Erro!',
					text: 'O campo Descrição precisa ser preenchido.',
					icon: 'error',
					confirmButtonText: 'OK'
				}).then(() => {
					$('#inputTextCTM_Descricao').focus(); // Coloca o foco no campo
				});
				camposValidos = false;
			}
		...
		function UpdateParcelas() {
			...
			$('#ParcPagTable tbody tr').each(function() {
				...
				var parcela = {
					...
					CTP_Descricao: linha.find('td:eq(1) input').val().trim(),
					CTP_DtaVencimento: linha.find('td:eq(2) input').val().trim(),
					CTP_Valor: linha.find('td:eq(3) input').val().replace(/[R$\.\s]/g, '').replace(',', '.').trim()
				};
		...
		$('#btnNovaParcela').click(function() {
			$('#ParcPagTable tbody').append(
				`<tr>
					<td>#</td>
					<td><input type="text" style="text-align: left;" placeholder="Descrição da Parcela" /></td>
