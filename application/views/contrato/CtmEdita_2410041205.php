...
;		document.getElementById('button-addon3').addEventListener('click', function() {
		...
		function ParcPagListar () {
				...
				"columns": [
					...
					},
					{
						...
						render: function(data, type, row) {
							return '<input type="text" style="text-align: right;" value="' + data + '" id="rowCTP_Valor' + row.CTP_Codigo + '" />';
						}
					}
				],
				"columnDefs": [
		...
		function ParcPagSomar() {
			...
			$('#ParcPagTable').find('tr').slice(1).each(function() {				
				vParcPagSomarTotalVal += parseFloat($(this).find('td:eq(3) input').val()) || 0;
			});
		...
		function setInputTextHints() {
			...
			$('[id^="rowCTP_CorrecaoIndice"]').prop('title', 'Informe o índice financeiro contratado para ajuste do valor da parcela.');
