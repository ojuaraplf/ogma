...
<body style="background: #eeeeee;">
    <div id="main-wrapper">
        ...
        <div class="page-wrapper d-flex flex-column min-vh-100">
								...
								<ul class="nav nav-tabs" role="tablist">
									<li class="nav-item">
										<a class="nav-link active" data-toggle="tab" href="#informacaoProdServ" role="tab">
											<span id="lspanAbaInfProdServ" class="hidden-xs-down">Informação Produto/Serviço</span>
										</a>
									</li>
									<li class="nav-item">
										<a class="nav-link" data-toggle="tab" href="#informacaoPagamento" role="tab">
											<span id="lspanAbaInfPagamento" class="hidden-xs-down">Informação de Pagamento</span>
										</a>
									</li>
								</ul>
											...
											<div class="col-12">
												<table id="RecTable" class="table table-hover" style="width:100%">
													<thead>
														<tr>
															<th id='coRecSeque'>#</th>
															<th id='coRecDescr'>Descrição</th>
															<th id='coRecUnida'>Unidade</th>
															<th id='coRecQuant'>Quantidade</th>
															<th id='coRecValUn'>Valor</br>Uni (R$)</th>
															<th id='coRecValUn'>Valor</br>Tot (R$)</th>
															<th>
																<button id="btnNovoRecurso" style="border: none; background: none; font-size: 14px; cursor: pointer;">
            ...
            <footer class="footer text-center mt-auto">
                © 2019 wDiscover Ltda - CtmEdita: vs00.05 202410101140
            </footer>
        </div>
    </div>
    ...
    <script type="text/javascript">
		...
		$('#btnSalvar').click(function() {
			...
			$.when(UpdateContratosMaster(), UpdateParcelas()).done(function(r1, r2) {
				Swal.fire(
					'Tudo certo!',
					'Contrato e parcelas atualizados com sucesso.',
					'success'
				).then(() => {
		...
		function validarCampos() {
			...
			var campos = [
				...
				{ id: '#inputTextCTM_ValorTotal', mensagem: 'O campo Valor Total precisa ser preenchido.' },
		...
		function UpdateParcelas() {
			...
			// Itera por todas as linhas da tabela de parcelas
			...
			$.ajax({
				...
				error: function(xhr, status, error) {
					Swal.fire(
						'Erro!',
						'Ocorreu um erro ao salvar as parcelas. Tente novamente.',
						'error'
					);
					console.log(xhr.responseText);
				}
		...
		function ParcPagListar () {
                ...
                "initComplete": function(settings, json) {
					...
					$(document).on('click', '.btnExcluirParcela', function() {
						...
						if (parcelaId) {
							linhasRemovidas.push(parcelaId);
						}
						linha.remove();
						console.table('console.table(linhasRemovidas)');
						console.table(linhasRemovidas);
						ParcPagSomar();
					});
		...
		$('#btnNovaParcela').click(function() {
			...
			$("input.parcela-valor").on('change', function() {
				ParcPagSomar();
			});
		...
		function setInputTextHints() {
			...
			$('#inputTextCTM_DataVigenciaFim, #inputTextCTM_DataVigenciaFim').prop('title', 'Data do término do período de vigência deste Contrato Master.');
