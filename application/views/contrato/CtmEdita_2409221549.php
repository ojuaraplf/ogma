...
<body style="background: #eeeeee;">
    <div id="main-wrapper">
        ...
        <div class="page-wrapper d-flex flex-column min-vh-100">
                            ...
                            <div class="card-body">
                               <div class="row mb-3">
									...
									<div class="col-md-2">
										...
										<input type="text" style="height: 38px; padding: 6px 12px;" class="form-control" value="<?php echo $ArrayEditaCTM[0]['CTM_DataRequisicao']; ?>" id="inputTextCTM_DataRequisicao" />
									</div>
									<div class="col-md-3">
										<label for="inputSelectCLI_Codigo" class="control-label col-form-label">Cliente</label>
									...
									<div class="col-md-3">
										<label for="inputSelectTCT_Codigo" class="control-label col-form-label">Tipo Contrato</label>
									...
									<div class="col-md-3">
										<label for="inputSelectSTT_Codigo" class="control-label col-form-label">Status</label>
								...
								<div class="row mb-3">
									<div class="col-6">
										<label for="inputTextCTM_LinkProposta" class="control-label col-form-label">Proposta</label>
									...
									<div class="col-6">
										<label for="inputTextCTM_LinkContrato" class="control-label col-form-label">Contrato Assinado</label>
										<div class="input-group">
								...
								<div class="row mb-3">
									...
									<div class="col-md-2">
										<label for="inputSelectCTM_UPCCodigo" class="control-label col-form-label">Unidade Padrão do Contrato</label>
            ...
            <footer class="footer text-center mt-auto">
                © 2019 wDiscover Ltda - SttEdita: vs00.05 20240920
            </footer>
        </div>
    </div>
    ...
    <script type="text/javascript">
		...
		$('#inputTextCTM_DataRequisicao').datepicker({
...
;		document.getElementById('button-addon3').addEventListener('click', function() {
		...
		$('#inputSelectCLI_Codigo').change(function() {
			var clienteCodigo = $(this).val(); // Obtém o valor selecionado do select
			console.log("Cliente selecionado: " + clienteCodigo); // Verifica o valor do cliente selecionado
			if (clienteCodigo) {
				buscarPessoasDoCliente(clienteCodigo); // Chama a função AJAX para buscar as pessoas do cliente
			}
		});
		...
		$('#inputSelectCTM_CLPCodigo').change(function() {
			...
			$.ajax({
				url: "<?php echo base_url('contrato/CtmLista/buscarDadosPessoaDoCliente'); ?>",
				type: "POST",
				data: {
					clpCodigo: clpCodigo,
				...
				success: function(response) {
					// Atualiza os campos de telefone e e-mail com a resposta do servidor
					$('#inputTextCLP_Email').val(response.CLP_Email);
					$('#inputTextCLP_Telefone1').val(response.CLP_Telefone1);
					$('#inputTextCLP_Telefone2').val(response.CLP_Telefone2);
				},
				error: function(xhr, status, error) {
					console.error("Erro ao buscar os dados da Pessoa do Cliente: ", error);
				}
			});
		});
		...
		function buscarPessoasDoCliente(clienteCodigo) {
			$.ajax({
				url: "<?php echo base_url('contrato/CtmLista/buscarPessoasDoCliente'); ?>",  // Corrija o caminho do controller aqui, se necessário
				type: "POST",
				data: { clienteCodigo: clienteCodigo },
				dataType: "json",
				success: function(response) {
					console.log(response); // Exibe no console para depuração
        ...
        function setInputTextHints() {
            ...
            $('#inputTextCTM_Codigo, #linputTextCTM_Codigo').prop('title', 'Código (Id) do Contrato Master.');
            $('#inputTextCTM_Descricao, #linputTextCTM_Descricao').prop('title', 'Descrição do Contrato Master.');
