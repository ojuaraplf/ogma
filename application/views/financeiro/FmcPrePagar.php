...
<body style="background: #eeeeee;">
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <div class="col-3">
                                        ...
                                        <input type="text" id="eOptionMes" class="form-control" />                                        
                                    </div>                                    

                                    <div class="col-5">
                                        <label for="comboboxGpo" class="text-left control-label col-form-label"> Gerente de Projeto </label>
                                        <select class="form-control" id="comboboxGpo">
                                            <!--
                                            <option value="0"> Todos os gerentes ... </option>
                                            <!?php foreach ($listaGpo as $item): ?>
                                    ...
                                    <div class="col-4">                                                                                
                                        <button class="btn float-right" style="font-size: 25px; color: #000000; background-color: #E6E6FA;" id="btnTurnoXtrabalho"> <i class="mdi mdi-clock-end"></i> </button>
                                    </div>
    ...
    <?php $this->load->view('modal/modalFmcTurnoXTrabalho') ?>
    ...
    <script type="text/javascript">
        ...
        $.when( PopulaComboboxGpo(MesAnoAnterior()) ).done(function(r1) {
            ListaNaMarra();
        });        
        ...
        function ListaNaMarra() {
            ...
            vData1 = Dia01DoMes($('#eOptionMes').val());
            vData2 = Dia30DoMes($('#eOptionMes').val());
        ...
        $('#eOptionMes').change(function() {            
            PopulaComboboxGpo(this.value);
        });

        function PopulaComboboxGpo(datathis) {

            // datathis = this.value;
            console.log('##################### 1');
            var UserCodigo = "<?php echo $this->session->userdata('userCodigo') ?>";            
            var datacerta = datathis.substr(3, 4) + '-' + datathis.substr(0, 2);
            console.log('console.log(datacerta)');
            console.log(datacerta);
            return $.ajax({
                url: "<?php echo base_url(); ?>financeiro/FmcLista/fetchselectedGco/" + datacerta,
                type: 'GET',
                dataType: 'JSON',
                success: function(response) {    
                    console.log('##################### 2');                
                    var optionsGcoItem = [];                    
                    optionsGcoItem.push('<option value="0"> Todos os GPs ... </option>');
                    response.listaGpo.forEach(function(e) {
                        optionsGcoItem.push('<option value="' + e.CODIGO + '" '+ (e.CODIGO==UserCodigo ? 'selected' : '') + '>' + e.NOME + ' </option>');                        
                    });
                    $('#comboboxGpo').html(optionsGcoItem);
                }
            });
        };
        
        function Dia01DoMes(pMesAno) {
            return (pMesAno).substring(3, 7)+'-'+(pMesAno).substring(0, 2)+'-01';
        }

        function Dia30DoMes(pMesAno) {            
            vUltDia = daysInMonth((pMesAno).substring(0, 2)-1,(pMesAno).substring(3, 7));
            return (pMesAno).substring(3, 7)+'-'+(pMesAno).substring(0, 2)+'-'+vUltDia;
        }

        $('#btnTurnoXtrabalho').click(function() {
            console.log('btnTurnoXtrabalho');
            $('#modalFmcTurnoXTrabalho').modal('show');
        });


        function setInputTextHints() {
            $('#btnTurnoXtrabalho').prop('title', 'Visualizar trabalho extraturno diário:\nPossíveis apontamentos sobrepostos num mesmo dia.');
            $('#colTabClbr').prop('title', 'Nome do Colaborador/consutor.');
            $('#colTabData').prop('title', 'Data.');
            $('#colTabTurn').prop('title', 'Turno na Data:\nTotal em horas do período trabalhado na Data.\nDiferença entre primeiro e último apontamentos na Data.');
            $('#colTabTrab').prop('title', 'Trabalho na Data:\nTotal em horas trabalhado na Data.');
            $('#colTabOver').prop('title', 'Tempo Extraturno:\nTotal em horas do tempo sobreposto entre apontamentos na Data.');

            $('#eOptionMes').prop('title', vHelpOptionMes );
            $('#comboboxGpo').prop('title', 'Selecione o GP do Plano de Serviço que teve trabalho no mês de referência.' );                    
