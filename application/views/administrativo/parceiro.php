...
<body style="background: #eeeeee;">
              ...
              <div class="card-body">
                ...
                <div class="row mb-3">
                  ...
                  <div class="col-2">
                    ...
                    <button class="btn btn-primary btn-block" id="btnx" style="display:none;">x</button>
                  </div>
                  ...
                  <input type="text" id="num_CnpjCpf" class="form-control "/>
                ...
                <div id="divSemelhantes" class="table-responsive">
                <table id="tableSemelhantes" style="text-align: center;" class="table table-hover table-sm table-bordered" >
                  <thead>
                    <tr style="text-align: center;" scope="col">
                      ...
                      <th style="width: 10%;" scope="col"> Colaborador</th>
                ...
                <table id="tableParceiros" style="text-align: center;" class="table table-hover table-sm table-bordered" >
                  <thead>
                    <tr style="text-align: center;" scope="col">
                      <th style="width: 10%;" scope="col"> VHSys ID</th>
                      <th style="width: 15%;" scope="col"> CPF/CNPJ</th>
  ...
  <script type="text/javascript">
    ...
    function listasemelhantes(strNome) {

      $("#tableSemelhantes > tbody").html("");

      
      var strPrimeiroNome = strNome.split(' ').shift(); 
      //alert(strPrimeiroNome);

      $.ajax({
				url: "<?php echo base_url(); ?>administrativo/parceiro/listasemelhantes",
                type: 'POST',
                dataType: 'json',  
                data: {
                  PRIMEIRONOME: strPrimeiroNome
                },
        ...
        error: function(e, ts, et) {
				...
				success: function (data) {
                    ...
                    for (var i = data.length - 1; i >= 0; i--) {
                        /*
                        var strstatus = "";
                        if (parseInt(data[i].CBR_REF) > ) {
                          var strstatus = '<span class="badge badge-primary">Primary</span>';
                        }
                        if (parseInt(data[i].CBR_PESempCodigo) > ) {
                          var strstatus = '<span class="badge badge-primary">Primary</span>';
                        }
                        */
                        html.push('<tr scope="row">');
                        html.push('<td>' + data[i].PES_Nome + ' (' + data[i].PES_Codigo + ')</td>');
                        html.push('<td>' + data[i].PES_CnpjCpf + '</td>');
                        html.push('<td>' + data[i].Login + '</td>');
                        html.push('<td>' + data[i].CBR_REF + '</td>');                      
                        html.push('<td>' + data[i].CBR_PESempCodigo + '</td>');
                        html.push('</tr>');

                        }

                    $('#tableSemelhantes').append(html);
    ...
    $('#btnx').click(function() {
      ...
      listasemelhantes($('#txtPES_Nome').val());
