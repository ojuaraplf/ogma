...
<body style="background: #eeeeee;">
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="table-responsive">
                                    ...
                                    <b>Versão Beta: 00.80 - 12/03/2023</b><br/>
                                </div>
    ...
    <script type="text/javascript">
        ...
        var ArrayAtgDetCons = [];
        var vTrabalhoId = '';
        var vTrabalhoVa = 0.00;
        var vTrabalhoCo = 'gray';
                                
        $.when(fetchComboAtf() ).done(function(r1) {
            
            for (var i = 0; i <= r1.length - 1; i++) {

                /*
                arrayAtf.push('<option value="' + r1[i].ATF_Codigo + '" ' + VarSelected + '> ' + r1[i].ATF_Descricao + ' </option>');
                */
                arrayAtf.push(r1[i]);    
            }            
        ...
        console.log('console.log(arrayAtf)');
        console.log(arrayAtf);
            ...
            table = $('#tableATG').DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        className: "text-center",
                        'render': function (data, type, row) {
                            return '<select id="ComboFamiliaAtg'+ row.ATG_CODIGO +'" class="form-control" >' + PopulaSelectAtf(row.ATF_CODIGO) + '</select>';
                        }
                    },                    
                    {
                        "data": "ATG_QTHORA",
                    ...
                    },
                    {
                        ...
                        className: "text-center",
                        render: function(data, type, row) {                            
                            ArrayAtgDetCons.push({
                                "ATG_CODIGO": row.ATG_CODIGO,
                                "ATG_CONSULTORES": row.CONSULTORES
                            });
                            vTrabalhoVa = data == null ? '0.00' : data;
                            vTrabalhoId = 'spanTRABALHO' + row.ATG_CODIGO;
                            vTrabalhoCo = data == null ? 'gray' : 'black';
                            return  '<span id="' + vTrabalhoId + '" style="color:'+ vTrabalhoCo +';" >' + vTrabalhoVa + '</span>';
                        }
                    },
                    {
                        "data": "INICIO",
        ...
        function SalvartableATG() {
            ...
            $('#tableATG').find('tr').slice(1).each(function(i, el) {
                ...
                var ATG_ATFCodigo = $tds.eq(3).find("select").val();
                var ATG_QTHORA = $tds.eq(4).find("input").val();
                var ATG_MomInicExecucaoInformado = $tds.eq(6).find("input").val();
                var ATG_MomFinalExecucaoInformado = $tds.eq(9).find("input").val();
                var ATG_PORCENTAGEMAPRONTADA = $tds.eq(10).find("input").val();
                var vATG_FlgConcluida = $tds.eq(11).find('input').is(":checked");
                var ATG_FlgConcluida = vATG_FlgConcluida == true ? 1 : 0;
                arrayTableATG.push({ATG_CODIGO:ATG_CODIGO, ATG_ATFCodigo:ATG_ATFCodigo, ATG_QTHORA:ATG_QTHORA, ATG_MomInicExecucaoInformado:ATG_MomInicExecucaoInformado, ATG_MomFinalExecucaoInformado:ATG_MomFinalExecucaoInformado, ATG_PORCENTAGEMAPRONTADA:ATG_PORCENTAGEMAPRONTADA, ATG_FlgConcluida:ATG_FlgConcluida });
            });
        ...
        function fetchComboAtf() {
            console.log('estou na fetchComboAtf()')
            return $.ajax({
                url: "<?php echo base_url(); ?>gestaoprojeto/GpoLista/FetchAtf",
                dataType: 'json',
                type: 'POST'
            });
        }
        ...
        function PopulaSelectAtf(pAtfCodigo) {

            ret_arrayAtf = [];
            for (var i = 0; i <= arrayAtf.length - 1; i++) {                
                ret_arrayAtf.push('<option value="' + arrayAtf[i].ATF_Codigo + '" '+ (arrayAtf[i].ATF_Codigo == pAtfCodigo ? 'selected' : '') + ' > ' + arrayAtf[i].ATF_Descricao + ' </option>');                
            }            
            return ret_arrayAtf;
        ...
        function setInputTextHints() {
            ...
            $('[id^=spanTRABALHO]').each(function(){                    
                $(this).prop('title', ArrayAtgDetCons.find(linha => linha.ATG_CODIGO == this.id.substr(12,6))["ATG_CONSULTORES"]);
            });
