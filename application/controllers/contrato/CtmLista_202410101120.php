...
class CtmLista extends CI_Controller {
    ...
    public function UpdateContratosMaster() {
        ...
        $dadosContrato = array(
            ...
            'CTM_CLPCodigo' => $this->input->post('CTM_CLPCodigo'),
            'CTM_CBRGestorWdCodigo' => $this->input->post('CTM_CBRGestorWdCodigo'),
            'CTM_CBRFinancWdCodigo' => $this->input->post('CTM_CBRFinancWdCodigo'),
        ...
        $this->lctm->UpdateContratosMaster($dadosContrato);
    }
	...
	public function UpdateParcelas() {
		...
		$linhasRemovidas = $this->input->post('linhasRemovidas');
		$CTM_Codigo = $this->input->post('CTM_Codigo');

		foreach ($parcelas as $parcela) {
			// Verifica se CTP_Codigo é vazio, null ou undefined
		...
		// Excluir as linhas removidas
		if (!empty($linhasRemovidas)) {
			foreach ($linhasRemovidas as $id) {
