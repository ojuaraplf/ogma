...
class FmcLista extends CI_Controller
{
    ...
    public function fetchselectedGco($aGcoMes){
        $data['listaGpo'] = $this->plm->fetchselectedGco($aGcoMes);
        echo json_encode($data);
    }

    public function FmcPrePagar()    
    {        
        // $pData1 = mktime(0, 0, 0, date('m')-1, 1, date('Y')-1);
        // $pData2 = mktime(23, 59, 59, date('m')+1, date('d')-date('j'), date("Y"));
        // $MesAnterior = date('Y-m-01', strtotime('-1 months', strtotime(date('Y-m-d'))));
        // print_r($MesAnterior);
        /*
        $GcoMes = date('Y-m-01', strtotime('-1 months', strtotime(date('Y-m-d'))));
        $data['GcoMes'] = $GcoMes;
        $data['listaGpo'] = $this->plm->fetchselectedGco($GcoMes);
        $this->load->view('financeiro/FmcPrePagar', $data);
        */
    ...
    {   
        $pGCOCodigo = $this->input->post("vGCOid");
        ...
        $data = $this->plm->fetchFmcPrePagar($pGCOCodigo, $pCBRCodigo, $pCLICodigo, $pPJTCodigo, $pATGCodigo, $pAgrupaCBR, $pAgrupaCLI, $pAgrupaPJT, $pAgrupaATG, $pAgrupaLCT, $pDataDe, $pDataAte );
        echo json_encode($data);
    }
	...
	{       
        ...
        $cbrInfoFaturamento = $this->plm->fetchInfoFaturamento($LinhaSalva[0]["FMC_Mes"], $LinhaSalva[0]["FMC_CBRCodigo"]);
        $LinhaSalva[0]["FMC_SomaContaCorrente"] = $cbrInfoFaturamento->CONTACORRENTE;
        $LinhaSalva[0]["FMC_VrSubtotal"] = $cbrInfoFaturamento->SUBTOTAL;
        $LinhaSalva[0]["FMC_VrTotal"] = $cbrInfoFaturamento->TOTAL;
    ...
    public function fetchTurnoXtrabalhoMes()
    {
        // Recebe os parâmetros POST
        $aData = $this->input->post("pData");
        $aDataLen = $this->input->post("pDataLen");
        $aCbr = $this->input->post("pCbr");

        // Verifica se o modelo está carregado
        if (!isset($this->plm)) {
            // Envia resposta JSON com erro
            echo json_encode(['error' => 'Modelo plm não está carregado.']);
            return;
        }

        // Busca os dados usando o modelo
        $data = $this->plm->fetchTurnoXtrabalhoMes($aData, $aDataLen, $aCbr);
        ...
        // Codifica os dados em JSON e retorna a resposta
        echo json_encode($data);
    }
}