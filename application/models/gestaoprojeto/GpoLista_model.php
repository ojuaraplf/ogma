...
class GpoLista_model extends CI_Model
{
    ...
    public function ogsv_PJT_ExtrProjeto01( $pData1, $pData2, $selCLI ) {
        
        $pData1 = $pData1 == NULL ? date("Y-m-d") : $pData1;
        $pData2 = $pData2 == NULL ? date("Y-m-d") : $pData2;
        $selCLI = $selCLI == 0 ? NULL : $selCLI;
        
        $query = $this->db->query("CALL ogsv_PJT_ExtrProjeto01(?, ?, ?)", array( $pData1, $pData2, $selCLI ));
    ...
    public function FetchAtf() {
        
        $query = $this->db->query("CALL ogsv_ATF_Lista01()");
