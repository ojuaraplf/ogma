...
class GpoLista_model extends CI_Model
{
    ...
    public function fetchselectedCbr( $aCLICodigo, $aPJTCodigo, $aPJFCodigo, $aMostraTudo )
    {        
        $pCLICodigo = $aCLICodigo == "0" ? null : $aCLICodigo;
        $pPJTCodigo = $aPJTCodigo == "0" ? null : $aPJTCodigo;
        ...
        $query = $this->db->query("CALL ogsv_CBR_Lista06(?, ?, ?, ?)", array( $pCLICodigo, $pPJTCodigo, $pPJFCodigo, $pMostraTudo ));
        $this->db->close();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
    ...
    public function FetchLctRelApp($pCLICodigo, $pPJTCodigo, $pPJFCodigo, $pCBRCodigo, $pPeriodoIni, $pPeriodoFim)
    {
        ...
        $query = $this->db->query("CALL ogfn_LCT_RelApp__(?, ?, ?, ?, ?, ?)", array($pCLICodigo, $pPJTCodigo, $pPJFCodigo, $pCBRCodigo, $pPeriodoIni, $pPeriodoFim));
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
            return array();
