...
class CbrLista_model extends CI_Model
{
    ...
    public function FechaMesFech($pCBRCodigo, $pMes, $pAbreProjeto)
    {

        $aCBRCodigo = $pCBRCodigo == 0 ? NULL : $pCBRCodigo;
	    $aMes = $pMes;
	    $aAbreProjeto = $pAbreProjeto == 0 ? NULL : $pAbreProjeto;

        $query = $this->db->query("CALL ogrh_FMC_ListaFechaMes(?, ?, ?)", array($aCBRCodigo, $aMes, $aAbreProjeto));
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
            return array();
