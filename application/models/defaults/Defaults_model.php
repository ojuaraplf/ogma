...
class Defaults_model extends CI_Model
{
  ...
  public function novaInteracao($data)
  {
	...
	public function fetchGrupoAtividadeFase($PJF_Codigo)
		{
  ...
  public function fetchogma_PES_Selecao02()
  {
    $query = $this->db->query('call ogma_PES_Selecao02("F");');
    if ($query->num_rows() > 0) {
      return $query->result_array();
    } else {
      return false;
  ...
  {
    $query = $this->db->query('call ogsv_PEQ_Selecao01(?, 1, 0);', array($PJF_CODIGO));
    if ($query->num_rows() > 0) {
      return $query->result_array();
    } else {
      return false;
